...
Esp32Camera::Esp32Camera(const esp_video_init_config_t& config) {
  ...
  if (false) { /* 用于构建 else if */
    ...
    video_device_name =
        ESP_VIDEO_USB_UVC_DEVICE_NAME(config.usb_uvc->uvc.uvc_dev_num);
  }
#endif
    ...
    if (i == 2) {
      ...
      switch (sensor_format_) {
        ...
        case V4L2_PIX_FMT_RGB565X: {
          ...
          // 目前 esp_video 的大小端都会返回格式为 RGB565，不会返回格式为
          // RGB565X，此 case 用于未来版本兼容
          auto src16 = (uint16_t*)mmap_buffers_[buf.index].start;
          auto dst16 = (uint16_t*)frame_.data;
      ...
      switch (frame_.format) {
        ...
        case V4L2_PIX_FMT_YUYV: {
          ESP_LOGW(TAG,
                   "YUYV format is not supported for PPA rotation, using "
          ...
          esp_imgfx_data_t convert_output_data = {
              .data = rotate_src,
              .data_len =
                  static_cast<uint32_t>(frame_.width * frame_.height * 3),
          };
        ...
        default:
          ESP_LOGE(TAG, "unsupported sensor format for PPA rotation: 0x%08x",
                   sensor_format_);
          if (ioctl(video_fd_, VIDIOC_QBUF, &buf) != 0) {
            ESP_LOGE(TAG, "Cleanup: VIDIOC_QBUF failed");
          }
...
std::string Esp32Camera::Explain(const std::string& question) {
  ...
  // 创建局部的 JPEG 队列, 40 entries is about to store 512 * 40 = 20480 bytes
  // of JPEG data
  ...
  encoder_thread_ = std::thread([this, jpeg_queue]() {
    ...
    image_to_jpeg_cb(
        ...
        [](void* arg, size_t index, const void* data, size_t len) -> size_t {
          ...
          JpegChunk chunk = {.data = (uint8_t*)heap_caps_aligned_alloc(
                                 16, len, MALLOC_CAP_SPIRAM),
                             .len = len};
